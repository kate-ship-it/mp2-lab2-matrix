...
public:
  ...
  {
      if (this != &v) {
          delete[] pMem;  // Освобождаем текущую память
          sz = v.sz;      // Переносим размер
          pMem = v.pMem;  // Переносим указатель на память
          v.sz = 0;       // Обнуляем исходный объект
          v.pMem = nullptr;
      }
      return *this;
  }
  ...
  T& operator[](size_t ind)
  {
      ...
      return pMem[ind]; // Возврат ссылки на элемент
  }
  //(константный доступ)
  const T& operator[](size_t ind) const
  {
      if (ind >= sz || ind < 0)
          throw out_of_range("Index out of range");
      return pMem[ind]; // Возврат константной ссылки
  }
...
public:
  ...
  TDynamicMatrix operator*(const TDynamicMatrix& m)
  {
      ...
      for (int i = 0; i < sz; i++) {         // По строкам первой матрицы
          for (int j = 0; j < sz; j++) {     // По столбцам второй матрицы
              for (int k = 0; k < sz; k++) { // Суммирование произведений
